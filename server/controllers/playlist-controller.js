...
deletePlaylistById = async(req, res) => {
    const list = await Playlist.deleteOne({ _id: req.params.id});
    if (list.deletedCount == 0){
        return res.status(400).json({ success: false, message: "list not deleted"});
    }
...
updatePlaylistById = async(req, res) => {
    const body = req.body;
     ...
     console.log("playlist: " + JSON.stringify(body))
     playlist.overwrite(new Playlist(body));
